...
RANGE_NAME = 'Sheet1!A:B'  # Columns A (Name) and B (Token)
CHECK_RANGE = 'Sheet1!A:A'  # Range to check for existing names (column A)
...
def check_duplicate_name(name):
    ...
    try:
        ...
        existing_names = [row[0].strip() for row in values if row]  # Assuming name is in column A
...
def draw_start_button(image):
    ...
    # Center the button on the screen (assuming 1920x1080 resolution)
...
def open_name_dialog():
    ...
    x_pos = ROI_WIDTH - 80
    y_pos = ROI_Y + ROI_HEIGHT -70  # Adjusted to be below webcam ROI
    ...
    name_window.attributes('-topmost', True)  # Ensure dialog stays above OpenCV window
    ...
    def save_name():
        ...
        if check_duplicate_name(temp_name):
            ...
            return  # Keep dialog open for next participant
        ...
        name_dialog_opened = False
        name_window.destroy()  # Close dialog only on successful registration
...
def mouse_callback(event, x, y, flags, param):
    global current_state, smile_start_time
    if event == cv.EVENT_LBUTTONDOWN:
        ...
        elif current_state == FINAL_DISPLAY and show_restart_button:
            button_text = "RESTART"
            font = cv.FONT_HERSHEY_SIMPLEX
            font_scale = 1.5
            thickness = 2
...
while True:
    try:
        ...
        if current_state == START:
            start_display = start_image.copy()
            start_display, start_button_coords = draw_start_button(start_display)
